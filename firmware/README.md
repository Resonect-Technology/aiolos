...
- **File Upload**: Upload the `firmware.bin` file generated by PlatformIO (located at `.pio/build/aiolos-esp32dev-debug/firmware.bin`).
- **Two-Layer Security**:
...
- **Access Method**: 
  1. Connect to the WiFi AP (e.g., "Resonect-Aiolos" with WiFi password from `OTA_PASSWORD`)
  2. Navigate to `http://192.168.4.1/update` in a web browser
  3. Enter credentials when prompted: username `admin`, password from `OTA_UPDATE_PASSWORD` in secrets.ini
