...
mode: agent
description:
  Guide for implementing backend support for new data points in the Aiolos
  Weather Station
tools:
  [
    'semantic_search',
...
Implement the backend support for a new data point transmitted from the Aiolos
Weather Station firmware, including database storage, API endpoints, and data
handling.
...
- The OpenAPI specification is generated automatically from the backend code, so
